    ...
    try {
      const blob = await buildStandardPdfBlob({
        ...
        render: ({ doc, margin, autoTable, headStyles }) => {
          ...
          y += 5
          doc.text(`Range di riferimento: ${range.min}ÔÇô${range.max}${unitSuffix}`, margin, y)
          y += 5
    ...
    return (
      <div className="flex-1 flex flex-col min-h-0 space-y-4 sm:space-y-6 px-4 sm:px-6 py-4 sm:py-6 max-w-[1800px] mx-auto w-full">
        <StaffAthleteSubpageHeader
          ...
          title={`Misurazione ÔÇö ${name || 'Atleta'}`}
    ...
    <>
      <div className="flex-1 flex flex-col min-h-0 space-y-4 sm:space-y-6 px-4 sm:px-6 py-4 sm:py-6 max-w-[1800px] mx-auto w-full">
        <StaffAthleteSubpageHeader
          backHref={backHref}
          backAriaLabel="Torna alle misurazioni"
          title={`${entry.label} ÔÇö ${name || 'Atleta'}`}
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0 space-y-1.5">
                ...
                <p className="text-text-tertiary text-xs">
                  Dal pi├╣ recente; solo rilevazioni con valore compilato
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-2 shrink-0">
                ...
                >
                  {pdfLoading ? 'GenerazioneÔÇª' : 'Esporta PDF'}
                </Button>
                <Button
                  type="button"
                  variant="ghost"
      ...
      <PdfCanvasPreviewDialog
        ...
        blob={pdfBlob}
        filename={pdfFilename}
        title={`Anteprima ÔÇö ${entry.label}`}
      />
    </>
  )
}
