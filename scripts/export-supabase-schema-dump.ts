...
async function trySupabaseDump(): Promise<boolean> {
  try {
    ...
    try {
      ...
      try {
        execSync(
          `npx supabase link --project-ref ${projectRef} --password "${SUPABASE_SERVICE_KEY.substring(0, 20)}..."`,
          {
            stdio: 'inherit',
            cwd: projectRoot,
          },
        ...
        console.log(
          '⚠️  Link fallito. Usa manualmente: npx supabase link --project-ref icibqnmtacibgnhaidlz\n',
        )
      }
    }

    execSync(`npx supabase db dump --schema public --linked -f "${schemaFile}"`, {
      stdio: 'inherit',
      cwd: projectRoot,
      env: { ...process.env },
    })
    ...
    console.log(
      '   2. Aver linkato il progetto: npx supabase link --project-ref icibqnmtacibgnhaidlz\n',
    )
    return false
  }
}
...
async function main() {
  ...
  writeFileSync(join(outputDir, 'HOW-TO-EXPORT-SCHEMA.sql'), instructionsSQL, 'utf-8')
  ...
  if (!success) {
    ...
    console.log(
      '  3. npx supabase db dump --schema public -f supabase-config-export/schema-complete.sql',
    )
