...
async function main() {
  ...
  console.log('\n2) profiles (email =):', profilesByEmail?.length ?? 0, e1 ? `| errore: ${e1.message}` : '')
  profilesByEmail?.forEach((p) => {
    console.log('   id:', p.id, '| user_id:', p.user_id, '| role:', p.role, '| is_deleted:', p.is_deleted)
  })
  ...
  for (const u of authUsers) {
    const { data: prof, error: e2 } = await admin
      ...
      .eq('user_id', u.id)
      .maybeSingle()
    console.log('\n3) profiles (user_id =', u.id + '):', prof ? '1 riga' : '0 righe', e2 ? `| errore: ${e2.message}` : '')
  ...
  console.log('\n4) inviti_atleti (email =):', inviti?.length ?? 0, e3 ? `| errore: ${e3.message}` : '')
  inviti?.forEach((i) => {
    console.log('   codice:', i.codice, '| stato:', i.stato, '| status:', i.status, '| expires_at:', i.expires_at)
  })
  ...
  const { data: org, error: e4 } = await admin
    ...
    .eq('slug', 'default-org')
    .maybeSingle()
  console.log('\n5) organizations (slug = default-org):', org ? org.id : 'nessuna', e4 ? `| errore: ${e4.message}` : '')

  console.log('\n--- Fine diagnostica ---')
  console.log('\nSe complete-profile fallisce:')
  console.log('- L\'API usa service_role (bypass RLS). Se vedi 0 righe in (2)/(3) per questa email, l\'INSERT può fallire per trigger (es. update_jwt_custom_claims) o vincoli.')
  console.log('- Esegui in Supabase SQL Editor: supabase_diagnose_registration_policies.sql per policy INSERT su profiles e possibili pulizie.')
}
