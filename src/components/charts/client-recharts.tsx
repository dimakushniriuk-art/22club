  ...
  return dynamic(
    async () => {
      try {
        ...
        // In Recharts 3 molti componenti sono forwardRef/memo (oggetti con $$typeof), non funzioni
        const isFunction = typeof Component === 'function'
        const isReactComponent =
          typeof Component === 'object' && Component !== null && '$$typeof' in Component
        if (!isFunction && !isReactComponent) {
          console.error(`Componente Recharts "${String(key)}" non è un componente valido`)
          const ErrorComponent = () => <ChartError />
          ErrorComponent.displayName = `ChartError_${String(key)}`
          return ErrorComponent
        }
