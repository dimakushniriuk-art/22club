...
/** Stile pulsanti circolari cyan (FAB periodo / nuovo appuntamento) — riusabile nell’header pagina staff. */
export const CALENDAR_FAB_BUTTON_CLASS =
...
export const CalendarView = forwardRef<CalendarViewHandle, CalendarViewProps>(
  function CalendarView(props, ref) {
    const {
      appointments,
      onEventClick,
    ...
    const getApi = useCallback(() => apiRef.current ?? calendarRef.current?.getApi?.(), [])
        ...
        <div className="flex flex-col gap-2.5 shrink-0 px-1 py-2 touch-manipulation max-sm:gap-2 sm:gap-3 sm:py-2.5 lg:flex-row lg:items-start lg:justify-between lg:gap-4 lg:px-1 lg:py-3 rounded-t-xl bg-gradient-to-r from-primary/10 via-transparent to-transparent border-b border-white/10 backdrop-blur-sm [overflow:visible]">
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-2 max-sm:justify-center sm:justify-start">
            ...
            <button
              data-action="today"
              onClick={handleToday}
              className="h-9 min-h-[44px] shrink-0 px-3 sm:px-4 rounded-lg border border-white/5 bg-background-secondary/40 text-text-primary text-sm font-medium hover:bg-primary/10 hover:border-primary/20 hover:text-primary transition-colors duration-200"
            >
              Oggi
            </button>
            <div className="flex shrink-0 items-center">
              <button
                data-action="prev"
                onClick={handlePrev}
                className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-full p-2 text-text-secondary transition-colors duration-200 hover:bg-primary/10 hover:text-primary"
              >
          ...
          <div className="flex w-full min-w-0 flex-col gap-2 self-stretch sm:flex-row sm:flex-wrap sm:items-center sm:gap-3 lg:w-auto lg:max-w-full lg:flex-row lg:justify-end lg:self-auto">
            {!compactToolbar && (
              <>
                ...
                <div
                  className="flex max-w-full flex-wrap items-center gap-1 rounded-lg border border-white/5 bg-background-secondary/40 p-1"
                  title="Step griglia"
                >
            ...
            <FullCalendar
              ...
              eventContent={(arg) => {
                ...
                if (arg.view.type === 'timeGridDay' || arg.view.type === 'timeGridWeek') {
                  ...
                  const lines = [esc(typeLabel), athlete ? esc(athlete) : '', timeStr].filter(
                    Boolean,
                  )
                  return {
