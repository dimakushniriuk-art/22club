...
export function ExerciseFormModal({
        ...
        if (editing.equipment) {
          ...
          if (equipmentList.length > 0) {
            ...
            const category = EQUIPMENT_CATEGORIES.find((cat) =>
              ...
              ((EQUIPMENT_BY_CATEGORY as any)[cat] as string[]).includes(firstEquipment),
            )
  ...
  useEffect(() => {
    if (!open || !openJustBecameTrueRef.current) return
    const initialized = editing ? form.id === editing.id : !form.name
  ...
  const isDirty = useMemo(() => {
    if (!open || !initialFormSnapshotRef.current) return false
    return JSON.stringify(form) !== initialFormSnapshotRef.current
  }, [open, form])
        ...
        return new Promise((resolve) => {
          ...
          supabase.storage
            ...
            .then((res: { data: unknown; error: unknown }) => {
              const { data: blob, error: downloadError } = res
