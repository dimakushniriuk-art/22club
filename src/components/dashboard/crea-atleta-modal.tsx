...
export function CreaAtletaModal({ open, onOpenChange, onSuccess }: CreaAtletaModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { isNativePlatform } = await import('@/lib/api-client')

      // Verifica se siamo su mobile
      if (isNativePlatform()) {
        throw new Error('Creazione atleta non disponibile su mobile. Usa la versione web.')
      }
      ...
      // Chiamata diretta all'API per gestire correttamente gli errori
      ...
      addToast({
        title: 'Creato',
        message: `Atleta ${formData.nome} ${formData.cognome} creato con successo`,
        variant: 'success',
      })
