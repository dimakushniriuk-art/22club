...
export function AdminUsersContent() {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      // Filtro ruolo
      if (roleFilter !== 'tutti') {
        if (roleFilter === 'athlete' && user.role !== 'athlete') {
          return false
        }
        if (roleFilter !== 'athlete' && user.role !== roleFilter) {
          return false
        }
      }
  ...
  const stats = useMemo(() => {
    ...
    const pt = users.filter((u) => u.role === 'trainer').length
    const atleti = users.filter((u) => u.role === 'athlete').length
  ...
  const handleResetPasswordConfirm = async (password: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const text = await response.text()
        let errorMessage = 'Errore nel reset della password'
        try {
            ...
            try {
              ...
              if (typeof errorData === 'object' && errorData !== null && 'error' in errorData) {
                ...
                const parts = [
                  ...
                  e.errorDetails ? `details=${String(e.errorDetails)}` : null,
                ].filter(Boolean)
  ...
  const getRoleBadge = (role: string) => {
    ...
    > = {
      ...
      nutrizionista: { label: 'Nutrizionista', variant: 'outline' },
      massaggiatore: { label: 'Massaggiatore', variant: 'outline' },
    }
    ...
    <div className="flex-1 flex flex-col space-y-4 sm:space-y-6 px-4 sm:px-6 py-4 sm:py-6 max-w-[1800px] mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/dashboard/admin/utenti/marketing">
            <Button
              variant="outline"
              className="bg-background-secondary border-border hover:bg-background-tertiary whitespace-nowrap"
            >
                          ...
                          >
                            ...
                            {user.role !== 'admin' && (
                              <DropdownMenuItem
                                ...
                                aria-label={`Impersona ${user.nome || user.email || user.id}`}
                              >
