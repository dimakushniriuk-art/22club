...
describe('TrainerSessionModal', () => {
  it('renders modal with Completamento Allenamento and two options', () => {
    ...
    render(<TrainerSessionModal open={true} onClose={onClose} onConfirm={onConfirm} />)
  ...
  it('calls onConfirm(true) when clicking Eseguito con Trainer', async () => {
    const onConfirm = vi.fn().mockResolvedValue(undefined)
    render(<TrainerSessionModal open={true} onClose={vi.fn()} onConfirm={onConfirm} />)
  ...
  it('disables action buttons when loading', () => {
    render(<TrainerSessionModal open={true} onClose={vi.fn()} onConfirm={vi.fn()} loading={true} />)
