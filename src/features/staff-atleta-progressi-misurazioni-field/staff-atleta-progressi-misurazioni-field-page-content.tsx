...
function MisurazioneFieldContent({
  ...
  const handleExportPDF = useCallback(async () => {
    if (!athlete || !entry || !range || listItems.length === 0) return
  ...
  const backHref = `${progressiBasePath}/misurazioni`
      ...
      <>
        <StaffAthleteProgressSubpageFrame
          header={{
            ...
            title: `Misurazione — ${displayName || 'Atleta'}`,
    ...
    <>
      <StaffAthleteProgressSubpageFrame
        header={{
          backHref,
          backAriaLabel: 'Torna alle misurazioni',
          title: `${entry.label} — ${displayName || 'Atleta'}`,
...
function MisurazioneFieldBody({
  routeParams,
}: {
  routeParams: Promise<{ id: string; field: string }>
}) {
  ...
  return (
    <StaffAthleteProgressBootstrap routeParams={routeParams}>
      {(context) => <MisurazioneFieldContent {...context} field={field} />}
    </StaffAthleteProgressBootstrap>
  )
}

export function StaffAtletaProgressiMisurazioneFieldPageContent({
  params,
}: {
  ...
  return (
    <StaffAthleteProgressSuspensePage params={params}>
      {({ routeParams }) => <MisurazioneFieldBody routeParams={routeParams} />}
    </StaffAthleteProgressSuspensePage>
  )
}
