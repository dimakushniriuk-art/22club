...
function StoricoAllenamentoEsercizioContent({
  ...
  const athleteProfileId = useMemo(() => {
    if (!isValidUserProfile || !user?.id) return null
    return isValidUUID(user.id) ? user.id : null
  }, [isValidUserProfile, user?.id])
...
export function HomeProgressiAllenamentiEsercizioPageContent({
  params,
}: {
