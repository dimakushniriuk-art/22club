...
import { WorkoutsPageSettingsDialog } from '@/features/staff-workouts/workouts-page-settings-dialog'
import { WorkoutsRegisteredAthletesStrip } from '@/features/staff-workouts/workouts-registered-athletes-strip'
...
export function WorkoutsShell({ slot1, slot2 }: Props) {
  ...
  const needsSlotAthleteDirectory = useMemo(
    () =>
      (!loading && Boolean(p1) && !eventForSlot1) || (!loading && Boolean(p2) && !eventForSlot2),
    [loading, p1, p2, eventForSlot1, eventForSlot2],
  )
  const registeredAthletesListEnabled =
    needsSlotAthleteDirectory || (deferSecondaryData && anagraficaStripVisible)
  ...
  const {
    ...
    listQueryEnabled: registeredAthletesListEnabled,
  })
