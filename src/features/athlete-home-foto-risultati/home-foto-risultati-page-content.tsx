...
export function HomeFotoRisultatiPageContent() {
  ...
  const { photos, loading, error, refresh, hasMore, loadMore } = useProgressPhotos({
    userId: profileId,
    role,
    angle: null,
  })
  ...
  const {
    visibleItems: visibleSessionKeys,
    hasMore: hasMoreSessions,
    loadMore: showMoreSessions,
  } = useProgressiveMount(sessionKeys, { initial: 8, step: 8 })
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (hasMoreSessions) showMoreSessions()
  ...
  }, [hasMore, hasMoreSessions, loadMore, showMoreSessions])
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          {loading && photos.length === 0 ? (
            ...
            <div className="space-y-6 sm:space-y-8">
              {visibleSessionKeys.map((sessionKey) => {
              ...
              <div ref={gallerySentinelRef} className="h-1" aria-hidden />
            </div>
          )}
        </div>
      </div>
