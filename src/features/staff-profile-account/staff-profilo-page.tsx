...
export function StaffProfiloPageContent() {
  ...
  const [deferSecondaryTabs, setDeferSecondaryTabs] = useState(false)

  useEffect(() => {
    if (!authUserId) {
      setDeferSecondaryTabs(false)
      return
    }
    const idleId =
      typeof window.requestIdleCallback === 'function'
        ? window.requestIdleCallback(() => setDeferSecondaryTabs(true))
        : undefined
    const timeoutId =
      idleId === undefined ? window.setTimeout(() => setDeferSecondaryTabs(true), 0) : undefined
    return () => {
  ...
  const {
    ...
    enabled: deferSecondaryTabs || activeTab === 'notifiche',
  })
