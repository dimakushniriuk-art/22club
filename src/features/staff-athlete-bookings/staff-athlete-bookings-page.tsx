...
import { useAthleteBookingsPage } from '@/hooks/use-athlete-bookings-page'
import { useResolvedParams } from '@/lib/next/use-resolved-params'

export default function StaffAthleteBookingsPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter()
  const resolved = useResolvedParams(params)

  const athleteId = resolved.id

  const { profile, appointments, loading, error } = useAthleteBookingsPage(athleteId)
