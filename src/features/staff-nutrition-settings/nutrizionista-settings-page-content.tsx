...
import { useNutrizionistaSettingsBootstrap } from '@/hooks/use-nutrizionista-settings-bootstrap'
import { useNutrizionistaSettingsPlanVersions } from '@/hooks/use-nutrizionista-settings-plan-versions'
import { useNutrizionistaSettingsVersionConfig } from '@/hooks/use-nutrizionista-settings-version-config'
import {
  Button,
  Input,
  Label,
...
import {
  ...
  type NutrizionistaSettingsAdaptiveSettings,
  type NutrizionistaSettingsAutoConfig,
...
type AutoConfig = NutrizionistaSettingsAutoConfig
type AdaptiveSettings = NutrizionistaSettingsAdaptiveSettings

export function NutrizionistaSettingsPageContent() {
  ...
  } = useNutrizionistaSettingsBootstrap(profileId)
  ...
  const {
    ...
    error: versionsError,
  } = useNutrizionistaSettingsPlanVersions(selectedPlanId || null)
  const {
    data: versionConfig,
    loading: versionLoading,
    error: versionConfigError,
    reload: reloadVersionConfig,
  } = useNutrizionistaSettingsVersionConfig(selectedVersionId || null)
  const [activeTab, setActiveTab] = useState<'structure' | 'adaptive'>('structure')
  const [saving, setSaving] = useState(false)
  const [saveError, setSaveError] = useState<string | null>(null)
  const [duplicateModalOpen, setDuplicateModalOpen] = useState(false)

  const [autoConfig, setAutoConfig] = useState<AutoConfig>(DEFAULT_NUTRIZIONISTA_AUTO_CONFIG)
  ...
  const error = saveError ?? bootstrapError ?? versionsError ?? versionConfigError

  useEffect(() => {
    if (!selectedVersionId) {
      setAutoConfig(DEFAULT_NUTRIZIONISTA_AUTO_CONFIG)
      setAdaptiveSettings(DEFAULT_NUTRIZIONISTA_ADAPTIVE_SETTINGS)
      setAutoConfigLoaded(false)
      setAdaptiveLoaded(false)
      return
    }
    if (versionLoading || !versionConfig) return
    setAutoConfig(versionConfig.autoConfig)
    setAdaptiveSettings(versionConfig.adaptiveSettings)
    setAutoConfigLoaded(versionConfig.autoConfigLoaded)
    setAdaptiveLoaded(versionConfig.adaptiveLoaded)
  }, [selectedVersionId, versionConfig, versionLoading])
  ...
  const applyVersionConfig = useCallback(
    async (versionId: string) => {
      const config = await queryClient.fetchQuery({
        queryKey: queryKeys.nutrition.settingsVersionConfig(versionId),
        queryFn: () => fetchNutrizionistaSettingsVersionConfig(supabase, versionId),
      })
      setAutoConfig(config.autoConfig)
      setAdaptiveSettings(config.adaptiveSettings)
      setAutoConfigLoaded(config.autoConfigLoaded)
      setAdaptiveLoaded(config.adaptiveLoaded)
    },
    [queryClient, supabase],
  )

  const _handleDuplicateFromPrevious = useCallback(async () => {
    const prevVersion = versions.find((version) => version.id !== selectedVersionId)
    if (!prevVersion) return
    try {
      await applyVersionConfig(prevVersion.id)
      setDuplicateModalOpen(false)
    } catch (e) {
      logger.error('Duplica da versione precedente', e)
      setSaveError(e instanceof Error ? e.message : 'Errore duplicazione')
    }
  }, [versions, selectedVersionId, applyVersionConfig])
            ...
            {versions
              ...
              .map((v) => (
                <Button
                  ...
                  onClick={() => {
                    void applyVersionConfig(v.id)
                      .then(() => setDuplicateModalOpen(false))
                      .catch((e) => {
                        logger.error('Duplica da versione', e)
                        setSaveError(e instanceof Error ? e.message : 'Errore duplicazione')
                      })
                  }}
                >
