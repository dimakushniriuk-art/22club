...
export function HomeTrainerPageContent() {
  const supabase = useSupabaseClient()
  const { loading: authLoading } = useAuth()
  const {
    data: trainerRow,
    isPending: trainerPending,
    isError: trainerQueryError,
    error: trainerQueryErr,
  } = useMyTrainerProfile(!authLoading)
  const trainer = useMemo((): TrainerProfile | null => {
    if (!trainerRow) return null
    return {
      pt_id: trainerRow.pt_id,
  ...
  const loading = trainerPending
  const [fullLoading, setFullLoading] = useState(false)
  const error = trainerQueryError
    ? ((trainerQueryErr as { message?: string } | null)?.message ?? 'Errore')
    : null
