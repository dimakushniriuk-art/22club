...
function MisurazioniContent({
  ...
  }, [athlete, progressData, setPdfLoading, openPdfWithBlob, notify])
    ...
    <>
      <StaffAthleteProgressSubpageFrame
        header={{
          ...
          title: `Misurazioni — ${displayName || 'Atleta'}`,
        ...
        <Card className={`relative overflow-hidden ${CARD_DS}`}>
          ...
          <CardContent className="relative z-10 p-4 pt-3 sm:p-6 sm:pt-4">
            {!athleteUserId ? (
              ...
              <MisurazioniValuesContent
                data={progressData}
                measurementDetailBasePath={`/dashboard/atleti/${profileId}/progressi/misurazioni`}
              />
            )}
          </CardContent>
        </Card>
...
function MisurazioniBody({ routeParams }: { routeParams: Promise<{ id: string }> }) {
  return (
    <StaffAthleteProgressBootstrap routeParams={routeParams}>
      {(context) => <MisurazioniContent {...context} />}
    </StaffAthleteProgressBootstrap>
  )
}

export function StaffAtletaProgressiMisurazioniPageContent({
  params,
}: {
  ...
  return (
    <StaffAthleteProgressSuspensePage params={params}>
      {({ routeParams }) => <MisurazioniBody routeParams={routeParams} />}
    </StaffAthleteProgressSuspensePage>
  )
}
