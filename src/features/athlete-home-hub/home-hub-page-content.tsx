...
export function HomeHubPageContent() {
  const { user } = useAuth()
  const profileId = user?.id ?? null
  const isAtleta = user?.role === 'athlete'
  const [deferSecondarySignals, setDeferSecondarySignals] = useState(false)

  useEffect(() => {
    if (!isAtleta || !profileId) {
      setDeferSecondarySignals(false)
      return
    }
    const idleId =
      typeof window.requestIdleCallback === 'function'
        ? window.requestIdleCallback(() => setDeferSecondarySignals(true))
        : undefined
    const timeoutId =
      idleId === undefined ? window.setTimeout(() => setDeferSecondarySignals(true), 0) : undefined
    return () => {
  ...
  const { inviti, refetch: refetchInviti } = useInvitiCliente(
    isAtleta ? profileId : null,
    deferSecondarySignals,
  )
