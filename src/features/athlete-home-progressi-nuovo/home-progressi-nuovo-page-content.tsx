...
export function HomeProgressiNuovoPageContent() {
  ...
  useEffect(() => {
    const loadLastMeasurement = async () => {
      try {
        ...
        const res = await fetch('/api/athlete/progress-logs?limit=1', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user?.id) {
        await invalidateProgressPhotosQueries(queryClient, user.id)
      }
