...
const ACTION_TYPES = Object.entries(MARKETING_AUTOMATION_ACTION_LABELS).map(([value, label]) => ({
  value,
  label,
}))

export function MarketingAutomationNewPageContent() {
  ...
  const { createAutomation } = useMarketingAutomationMutations()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createAutomation.mutateAsync(insert)
