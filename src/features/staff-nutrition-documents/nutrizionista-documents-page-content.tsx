...
import { useNutrizionistaDocumentsList } from '@/hooks/use-nutrizionista-documents-list'
import {
  Button,
  Input,
  Label,
...
export function NutrizionistaDocumentsPageContent() {
  ...
  const handleUpload = useCallback(async () => {
    ...
    if (!myOrgId && !myOrgIdText) {
      setSaveError('Org mancante: impossibile caricare.')
      return
    }
    setUploading(true)
    setSaveError(null)
    try {
      ...
      setSaveError(e instanceof Error ? e.message : 'Errore upload')
    } finally {
      setUploading(false)
    }
  }, [
  ...
  const handleSaveNote = useCallback(
    ...
    [editNoteValue, supabase, reload, queryClient],
  )
