...
import { useAthleteStatoCliente } from '@/hooks/use-athlete-stato-cliente'
...
function AppuntamentiPageContent() {
  ...
  const { data: statoCliente = null } = useAthleteStatoCliente(profileId, isAthlete)
  ...
  const appointmentsListEnabled = !isAthlete && Boolean(profileId)

  // Lista appuntamenti (solo ruoli non-atleta)
  const {
    appointments,
    loading,
    error,
    ...
    enabled: appointmentsListEnabled,
  })
