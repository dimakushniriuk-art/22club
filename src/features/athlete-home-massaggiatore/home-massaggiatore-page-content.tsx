...
function MassaggiatorePageContent() {
  ...
  const [loadMassageData, setLoadMassageData] = useState(false)
  useEffect(() => {
    if (!athleteUserId) {
      setLoadMassageData(false)
      return
    }
    const idleId =
      typeof window.requestIdleCallback === 'function'
        ? window.requestIdleCallback(() => setLoadMassageData(true))
        : undefined
    const timeoutId =
      idleId === undefined ? window.setTimeout(() => setLoadMassageData(true), 0) : undefined
    return () => {
  ...
  const { trattamenti, preferenze } = useAthleteMassageStats(athleteUserId, loadMassageData)
...
export function HomeMassaggiatorePageContent() {
  return (
    <Suspense fallback={null}>
