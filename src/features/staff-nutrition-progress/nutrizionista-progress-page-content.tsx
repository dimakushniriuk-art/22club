...
import { useNutrizionistaProgressOverview } from '@/hooks/use-nutrizionista-progress-overview'
import {
  Button,
  Input,
  Label,
...
export function NutrizionistaProgressPageContent() {
  ...
  const handleNuovoProgressoSubmit = useCallback(async () => {
    ...
    if (!athleteUserId) {
      setSaveError('Atleta senza account: impossibile salvare in progressi.')
      return
    }
    setSubmitting(true)
    try {
      ...
      setSaveError(e instanceof Error ? e.message : 'Errore inserimento')
    } finally {
      setSubmitting(false)
    }
  }, [
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setSaveError(e instanceof Error ? e.message : 'Errore estrazione PDF')
                      } finally {
