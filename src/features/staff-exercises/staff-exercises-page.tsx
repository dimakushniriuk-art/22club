...
export default function StaffExercisesPage() {
  ...
  const { items, loading, loadError, reload } = useStaffExercisesList()
  ...
  const [view, setView] = useState<'grid' | 'table'>(getStoredStaffExercisesView)
  const [sortField, setSortField] = useState<
    'name' | 'muscle_group' | 'equipment' | 'difficulty' | 'updated_at'
  >(getStoredStaffExercisesSort().field)
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>(
    getStoredStaffExercisesSort().direction,
  )
  ...
  }, [exerciseToDelete, addToast, reload])
  ...
  useEffect(() => {
    localStorage.setItem(STAFF_EXERCISES_VIEW_KEY, view)
  }, [view])
  useEffect(() => {
