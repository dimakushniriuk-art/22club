...
export function HomeProgressiFotoPageContent() {
  ...
  const { photos, loading, error, hasMore, loadMore, refresh } = useProgressPhotos({
    userId: profileId,
    role,
    angle: selectedAngle,
  })
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-background">
      <div className="min-h-0 flex-1 overflow-auto px-3 pb-24 safe-area-inset-bottom sm:px-4 md:px-6 space-y-4 md:space-y-5">
        ...
        {photos.length === 0 ? (
          ...
          <div className="space-y-4">
            <CompareDatesCard
              uniqueDates={visibleCompareDates}
              selectedDates={selectedDates}
              onDateSelect={handleDateSelect}
              getPhotosForDate={getPhotosForDate}
            />
            {hasMoreCompareDates ? (
              <Button type="button" variant="outline" onClick={showMoreCompareDates}>
                Mostra altre date
              </Button>
            ) : null}
