        ...
        try {
          ...
          if (profileError || !profileData) {
            ...
            if (code === 'PGRST116' || isLockOrAbort) {
              router.replace(buildWelcomePath(readPendingInviteCodice()))
              return
            }
            if (code === '42501') {
              logger.warn(
          ...
          const profile = profileData as LoginProfileForRedirect
          const redirectedFrom = searchParams.get('redirectedFrom')
          const athleteReturnPath = resolveAthleteRedirectFromLoginQuery(profile, redirectedFrom)
          if (athleteReturnPath) {
            router.replace(athleteReturnPath)
            return
          }
          const postLoginPath = resolvePostLoginRedirectPath(profile)
          if (!postLoginPath) {
            setError(getUnknownLoginRoleError(profile.role))
            setLoading(false)
            return
          }
...
export function AuthLoginPageContent() {
  return (
    <div
