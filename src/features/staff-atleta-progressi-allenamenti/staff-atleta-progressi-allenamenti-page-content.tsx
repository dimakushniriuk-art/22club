...
function AllenamentiContent({
  ...
  const handleExportPDF = useCallback(async () => {
    if (!data || data.total_exercises === 0) return
  ...
  }, [athlete, data, setPdfLoading, openPdfWithBlob, notify])
    ...
    <>
      <StaffAthleteProgressSubpageFrame
        header={{
          ...
          title: `Statistiche allenamenti — ${displayName || 'Atleta'}`,
          description:
            'Tutti gli stati sessione: pesi, tempi e serie anche in corso o senza metriche (come in app Home)',
        }}
      >
          ...
          <CardContent className="relative z-10 p-4 pt-3 sm:p-6 sm:pt-4">
            {!athleteUserId ? (
              ...
              <WorkoutExerciseCharts
                data={data}
                detailBasePath={`/dashboard/atleti/${profileId}/progressi/allenamenti`}
              />
            )}
          </CardContent>
        </Card>
...
function AllenamentiBody({ routeParams }: { routeParams: Promise<{ id: string }> }) {
  return (
    <StaffAthleteProgressBootstrap routeParams={routeParams}>
      {(context) => <AllenamentiContent {...context} />}
    </StaffAthleteProgressBootstrap>
  )
}

export function StaffAtletaProgressiAllenamentiPageContent({
  params,
}: {
  ...
  return (
    <StaffAthleteProgressSuspensePage params={params}>
      {({ routeParams }) => <AllenamentiBody routeParams={routeParams} />}
    </StaffAthleteProgressSuspensePage>
  )
}
