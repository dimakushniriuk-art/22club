...
import { InvitationsGrid } from '@/components/dashboard/invitations-grid'
...
const SORT_OPTIONS: { value: InvitoSort; label: string }[] = [
...
export default function StaffInviteAthletePage() {
  ...
  const [showConfirmExitCreateForm, setShowConfirmExitCreateForm] = useState(false)

  // Stato configurazione email (Resend) quando si apre il form invito
  useEffect(() => {
    if (!showCreateForm) return
    fetch('/api/invitations/email-status')
      .then((r) => r.json())
      .then((data) => setEmailConfigured(data?.configured === true))
  ...
  const handleCreateInvitation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newInvito = await createInvitation({
        ...
        giorni_validita: formData.giorni_validita,
      })

      // Se richiesto, invia email tramite API (lato server, così la API key Resend non è esposta)
      if (sendEmail && formData.email && newInvito) {
        try {
          ...
          if (!codiceInvito) {
            ...
            const registrationLink = `${window.location.origin}/registrati?codice=${encodeURIComponent(codiceInvito)}`
            ...
            if (!res.ok) {
              ...
              addToast({
                ...
                title: 'Invito creato',
                message:
                  "L'invio dell'email non è riuscito. Puoi copiare il link dalla lista e inviarlo manualmente.",
              })
            }
          }
        } catch (emailError) {
  ...
  const handleCopyLink = useCallback(
    async (code: string) => {
      const link = `${window.location.origin}/registrati?codice=${encodeURIComponent(code)}`
  ...
  const getStatusBadge = useCallback((stato: string) => {
    switch (stato) {
      case 'registrato':
        return (
  ...
  const toggleSelectId = useCallback((id: string, checked: boolean) => {
    setSelectedIds((prev) => {
  ...
  const handleRequestCloseCreateForm = useCallback(
    (open: boolean) => {
      if (!open && isFormDirty) {
        setShowConfirmExitCreateForm(true)
      } else {
        if (!open) {
          setFormErrors({})
          setFormData({ nome_atleta: '', email: '', giorni_validita: 7 })
        }
        handleCloseCreateForm(open)
      }
    },
    [isFormDirty, handleCloseCreateForm],
  )

  const handleConfirmExitCreateForm = useCallback(() => {
    setShowConfirmExitCreateForm(false)
    setFormErrors({})
    setFormData({ nome_atleta: '', email: '', giorni_validita: 7 })
    handleCloseCreateForm(false)
  ...
  const handleShowQR = useCallback((invitation: Invitation) => {
  ...
  return (
    ...
    >
            ...
            {formData.email && (
              <div className="space-y-2">
                ...
                {emailConfigured === false && (
                  <p className="text-text-tertiary rounded border border-amber-500/30 bg-amber-500/10 p-2 text-xs">
                    Per inviare email configura nel server: RESEND_API_KEY, RESEND_FROM_EMAIL,
                    RESEND_FROM_NAME (es. in .env.local).
                  </p>
                )}
              </div>
            )}
      ...
      {/* Conferma uscita da form con modifiche non salvate */}
      <AlertDialog open={showConfirmExitCreateForm} onOpenChange={setShowConfirmExitCreateForm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Modifiche non salvate</AlertDialogTitle>
            <AlertDialogDescription>
              Hai compilato dei campi senza creare l&apos;invito. Uscendo perderai le modifiche.
              Vuoi uscire?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
