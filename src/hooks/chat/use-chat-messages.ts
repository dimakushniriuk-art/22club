...
export function useChatMessages(
  ...
  const fetchMessages = useCallback(
    ...
    [getCurrentProfileId, onMessageUpdate, onError],
  )
  ...
  const deleteMessage = useCallback(
    async (messageId: string) => {
      try {
        ...
        // DELETE con filtro sender_id: la policy RLS (solo mittente) è l'unica verifica.
        // Evitiamo il fetch preliminare perché la SELECT può fallire per RLS (es. stesso org).
        ...
        if (!deleted?.length) {
          throw new Error('Messaggio non trovato o non eliminabile')
        }
