...
export function useAllenamenti(filters?: Partial<AllenamentoFilters>, sort?: AllenamentoSort) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // Profili creatori schede (created_by_profile_id = profiles.id)
      const uniqueCreatorProfileIds = new Set<string>()
      data.forEach((item) => {
        if (item.scheda?.created_by_profile_id) {
          uniqueCreatorProfileIds.add(item.scheda.created_by_profile_id)
        }
      })

      const trainerProfilesMap = new Map<string, { nome: string; cognome: string }>()
      if (uniqueCreatorProfileIds.size > 0) {
        try {
          const profileIdsArray = Array.from(uniqueCreatorProfileIds)
          logger.debug('Recupero trainer profiles', { count: profileIdsArray.length })
          ...
          } else if (trainerProfiles?.length) {
            trainerProfiles.forEach((profile) => {
              trainerProfilesMap.set(profile.id, {
                nome: profile.nome ?? '',
      ...
      const dataWithTrainers = data.map((item) => {
        if (item.scheda?.created_by_profile_id) {
          const trainer = trainerProfilesMap.get(item.scheda.created_by_profile_id)
          if (trainer) {
            return {
              ...item,
              scheda: {
