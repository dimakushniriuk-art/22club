...
describe('usePayments', () => {
  ...
  it('should fetch payments on mount', async () => {
    const { result } = renderHook(() => usePayments({ userId: 'test-user-id', role: 'trainer' }))
  ...
  it('should filter payments by athlete role', async () => {
    const { result } = renderHook(() => usePayments({ userId: 'athlete-1', role: 'athlete' }))
  ...
  it('should not fetch if userId is missing', () => {
    const { result } = renderHook(() => usePayments({ userId: null, role: 'trainer' }))
