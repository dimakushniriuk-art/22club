...
interface UsePaymentsProps {
  ...
  /** Solo admin: include pagamenti soft-deleted (deleted_at non null). Default false. */
...
export function usePayments({
  ...
  const fetchPayments = useCallback(async () => {
    try {
      ...
      } else if ((role === 'admin' || role === 'trainer') && userId) {
      ...
      const transformedData =
        ...
        (data as any[])?.map((payment: any) => ({
          ...
          created_by_staff_name:
            `${payment.created_by_staff?.nome || ''} ${payment.created_by_staff?.cognome || ''}`.trim(),
        })) || []
  ...
  }, [userId, role, enablePagination, includeDeleted, currentPage, pageSize])
