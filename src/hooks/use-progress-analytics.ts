...
/**
 * @param athleteId - profiles.user_id (auth.users.id). progress_logs.athlete_id è FK a profiles.user_id; i chiamanti devono passare user_id.
 */
export function useProgressAnalytics(athleteId?: string) {
  ...
  const fetchProgressData = useCallback(async (): Promise<ProgressKPI> => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const weekWorkouts = safeWorkouts.filter((workout: { created_at: string | null }) => {
        const raw = workout.created_at ?? ''
        const workoutDate = new Date(raw)
