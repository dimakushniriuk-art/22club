...
export function useMarketingKpi(enabled: boolean) {
  const router = useRouter()
  const query = useQuery({
    queryKey: queryKeys.marketing.kpi,
    queryFn: async () => {
      const json = await fetchMarketingJson<{ data: AthleteMarketingMetricEnriched[] }>(
        '/api/marketing/kpi',
      )
      return json.data ?? []
    },
