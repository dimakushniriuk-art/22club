...
export function useMarketingAnalytics(enabled: boolean) {
  const router = useRouter()
  const query = useQuery({
    queryKey: queryKeys.marketing.analytics,
    queryFn: () => fetchMarketingJson<AnalyticsPayload>('/api/marketing/analytics'),
    enabled,
    staleTime: STALE_MS,
