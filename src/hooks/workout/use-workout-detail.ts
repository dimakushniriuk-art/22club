...
export function useWorkoutDetail(workoutId: string | null, open: boolean) {
  ...
  const fetchWorkoutDetail = useCallback(async () => {
    ...
    try {
      ...
      // Ricostruisci items e circuitList da circuit_block_id per modifica scheda
      const circuitListBuilt: Array<{ id: string; params: WorkoutDayExerciseData[] }> = []
      for (let d = 0; d < transformedWorkout.days.length; d++) {
        const day = transformedWorkout.days[d]
        const exercises = day.exercises ?? []
        const items: DayItem[] = []
        let i = 0
        while (i < exercises.length) {
          const ex = exercises[i]
          const blockId = ex.circuit_block_id ?? null
          const toParam = (e: (typeof exercises)[0]): WorkoutDayExerciseData => ({
            exercise_id: e.exercise_id ?? '',
            sets: e.target_sets ?? 1,
          ...
          if (blockId == null) {
            items.push({ type: 'exercise', exercise: toParam(ex) })
            i++
          } else {
            const group: WorkoutDayExerciseData[] = []
            while (i < exercises.length && (exercises[i].circuit_block_id ?? null) === blockId) {
              group.push(toParam(exercises[i]))
              i++
            }
            circuitListBuilt.push({ id: blockId, params: group })
            items.push({ type: 'circuit', circuitId: blockId })
          }
        }
