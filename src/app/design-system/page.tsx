  ...
  return (
    <div className="min-h-screen min-w-0 bg-black text-text-primary">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-black/95 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)] backdrop-blur supports-backdrop-filter:bg-black/80">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-2 sm:gap-3 px-3 py-3 sm:px-4 sm:py-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-1.5 sm:gap-2">
            <Button variant="outline" size="sm" onClick={handleDownloadCss} className="border-white/10 bg-transparent text-text-secondary hover:bg-white/10 hover:text-text-primary shrink-0 text-xs sm:text-sm min-h-[40px] sm:min-h-0 px-2 sm:px-3">
              <FileText className="mr-1 sm:mr-1.5 h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />
              Scarica CSS
            </Button>
            <Button variant="outline" size="sm" onClick={handleDownloadReact} className="border-white/10 bg-transparent text-text-secondary hover:bg-white/10 hover:text-text-primary shrink-0 text-xs sm:text-sm min-h-[40px] sm:min-h-0 px-2 sm:px-3">
              <Copy className="mr-1 sm:mr-1.5 h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />
              Scarica React
            </Button>
            <Button variant="outline" size="sm" onClick={handleDownloadPdf} disabled={pdfLoading} className="border-white/10 bg-transparent text-text-secondary hover:bg-white/10 hover:text-text-primary disabled:opacity-50 shrink-0 text-xs sm:text-sm min-h-[40px] sm:min-h-0 px-2 sm:px-3">
              {pdfLoading ? <Loader2 className="mr-1 sm:mr-1.5 h-3.5 w-3.5 sm:h-4 sm:w-4 animate-spin shrink-0" /> : <Download className="mr-1 sm:mr-1.5 h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />}
              {pdfLoading ? 'Generazione...' : 'Scarica PDF'}
            </Button>
            <Link href="/" className="shrink-0 whitespace-nowrap rounded-lg border border-white/10 px-2.5 py-2 sm:px-3 text-xs sm:text-sm text-text-secondary transition-colors hover:bg-white/10 hover:text-text-primary min-h-[40px] sm:min-h-0 flex items-center justify-center">
              ← Torna
            </Link>
          </div>
        </div>

        <nav className="mx-auto max-w-5xl overflow-x-auto overflow-y-hidden px-3 pb-2 sm:px-4 sm:pb-3 -webkit-overflow-scrolling-touch" aria-label="Design system">
      ...
      <main className="mx-auto max-w-5xl min-w-0 space-y-10 sm:space-y-16 px-3 py-6 sm:px-4 sm:py-10">
        ...
        <section id="principi" className="scroll-mt-24 rounded-lg border border-white/10 bg-gradient-to-b from-zinc-900/95 to-black/80 p-4 sm:p-6 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04),0_4px_24px_-4px_rgba(0,0,0,0.5)]">
          ...
          <p className="text-xs sm:text-sm text-text-secondary break-words">
            Riempimento unico (no gradienti bicolore), bordo con sfumatura visibile (inset highlight), forme coerenti: <code className="rounded border border-primary/20 bg-primary/5 px-1.5 py-0.5 font-mono text-xs text-primary">rounded-md</code> per form, <code className="rounded border border-primary/20 bg-primary/5 px-1.5 py-0.5 font-mono text-xs text-primary">rounded-lg</code> per card e pulsanti.
          </p>
        </section>
      ...
      <footer className="border-t border-white/10 py-4 sm:py-6 px-3 text-center text-xs sm:text-sm text-text-muted">
        <p className="max-w-xl mx-auto">22Club Design System · Riferimento per fondazioni, componenti e pattern · <span className="text-primary/80">22club</span></p>
      </footer>
    </div>
  )
}
