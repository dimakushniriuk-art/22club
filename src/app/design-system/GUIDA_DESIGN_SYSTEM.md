...
- **Sotto-sezione "Elementi di design solo in queste pagine"** (solo per gruppo Dashboard — Nutrizionista): (1) StaffContentLayout tema teal (titolo gradient from-teal-400 via-cyan-400 to-blue-400), (2) Card lista/KPI: `border border-border`, `bg-background-secondary/80`, `ring-1 ring-white/5`, shadow inset leggero; stato attivo KPI con accento teal.
...
- **Messaggio informativo in card** (`LoginCard` prop `infoMessage`): box `border-primary/30 bg-primary/10` con icona `Info`, `role="status"`, usato quando l’utente arriva da area protetta o sessione scaduta (`/login?reason=session_expired`, `reason=auth_required`, `redirectedFrom=…`).

### Cross-device (shell globale)

- **`ConnectionStatusBanner`** (`src/components/shared/connection-status-banner.tsx`): in root layout sotto i provider; barra fissa in alto con `safe-area-inset-top`, stati offline (amber) / connessione ripristinata (emerald), `aria-live="polite"`.
- **Viewport** (`src/app/layout.tsx`): zoom utente consentito (`userScalable: true`, `maximumScale: 5`) per accessibilità.
- **PWA** (`public/manifest.json`): `orientation: "any"` per uso tablet anche in landscape.
- **Staff mobile header** (`DashboardMobileNav`): `pt-[env(safe-area-inset-top)]` sotto il notch; **`RoleLayout` `main`**: su staff `pt-0 md:pt-[env(safe-area-inset-top)]` per evitare doppio padding sotto il notch (header mobile vs sidebar desktop).
- **CSS globale** (`globals.css`): `:focus-visible` con outline primary; su viewport stretto `scroll-margin-bottom` su `input`/`textarea`/`select` a focus per tastiera virtuale; utility `.readable-prose-max` (`max-width: 65ch`) per blocchi di testo lunghi dove serve.
- **`prefers-reduced-motion`**: in `globals.css` (durate animazione/transition ridotte, `scroll-behavior: auto`, shimmer disattivato); componenti `FadeIn` / `SlideUp` / `ScaleOnHover` / `PulseIndicator` / `BounceIn` con classi `motion-reduce:*` (Tailwind).
- **Dialog** (`components/ui/dialog.tsx`): `Escape` chiude; focus trap nel pannello (Tab ciclico); ripristino focus all’elemento precedente alla chiusura; `DialogTrigger` attivabile con Enter/Spazio.
- **Installazione PWA** (`InstallPwaPrompt`): banner in basso solo su `/home` e `/dashboard` (non in dev, non se già standalone); Chrome/Android `beforeinstallprompt`; iOS: suggerimento “Condividi → Aggiungi alla schermata Home” dopo ~12s; dismiss con localStorage.
- **Service Worker** (`public/sw.js`): precache minimo; **`fetch` navigazioni `network-first`**, offline fallback su cache o `/login`; cache bump `22club-v3`.
- **Testi legali**: accordion Privacy/Termini con contenuto in `.readable-prose-max` per line-length leggibile.
- **Cookie / consenso** (`CookieConsent`, `src/lib/cookie-consent-storage.ts`): preferenze in `localStorage` (`22club-cookie-preferences`, v1: `analytics`, `functional`, `decidedAt`); migrazione da `cookie-consent` legacy; azioni **Solo necessari** / **Accetta tutto** / **Personalizza** (Sentry analitica + funzioni opzionali); dopo la scelta nessun controllo fisso in schermata; modifica preferenze da Impostazioni (vedi `CookiePreferencesSettingRow`); ancoraggio `/privacy#cookie-policy`. Pannello **Gestisci** (modale): focus trap, **Esc** per chiudere, `aria-live` per conferma salvataggio senza reload. Evento globale `OPEN_COOKIE_PREFERENCES_EVENT` + `requestOpenCookiePreferences()` per aprire il pannello da altre schermate. **`CookiePreferencesSettingRow`** (`src/components/shared/cookie-preferences-setting-row.tsx`): blocco riusabile (Impostazioni tab Privacy, Profilo atleta, Impostazioni nutrizionista) con link Cookie Policy e bottone che dispatcha l’evento. Sentry client (`instrumentation-client.ts`): replay/traces/log/PII solo se `analytics === true`; in produzione sample rate più bassi; senza consenso restano gli errori con `exception` in `beforeSend`. **InstallPwaPrompt** visibile solo con consenso funzionale; `beforeinstallprompt` registrato subito per non perdere l’evento. Retry rete transitoria su salvataggio impostazioni: `withNetworkRetry` in `src/lib/network-retry.ts` usato da `use-user-settings`.
