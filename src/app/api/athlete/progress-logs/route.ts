  ...
  try {
    ...
    if (role !== 'athlete' && role !== 'atleta') {
      return NextResponse.json(
        { error: 'Solo atleti possono consultare lo storico da qui' },
        { status: 403 },
      )
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (role !== 'athlete' && role !== 'atleta') {
      return NextResponse.json(
        { error: 'Solo atleti possono registrare misurazioni da qui' },
        { status: 403 },
      )
    }
    ...
    if (sessionInsert.error && isRlsDenied(sessionInsert.error.code, sessionInsert.error.message)) {
      sessionInsert = await supabase.from('progress_logs').insert({ ...baseRow })
    }
      ...
      return NextResponse.json(
        {
          ...
          hint: 'Controlla in pg_policies se esistono policy INSERT restrictive su progress_logs. La policy SELECT atleta usa spesso profiles.id = athlete_id (inconsistente se athlete_id è user_id): allinea le policy o verifica permissive/restrictive.',
        },
        { status: 403 },
      )
    }
