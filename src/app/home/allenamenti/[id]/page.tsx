...
function SchedaAllenamentoContent() {
  ...
  const startHref = useMemo(
    () => `/home/allenamenti/oggi?workout_plan_id=${encodeURIComponent(planId ?? '')}`,
    [planId],
  )
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-background">
      <AllenamentiPageHeader
        ...
        onBack={() => router.push('/home/allenamenti')}
        withBottomMargin
      />
      <div className="min-h-0 flex-1 overflow-auto px-3 pb-28 safe-area-inset-bottom sm:px-4 min-[834px]:px-6 min-[834px]:pb-24">
            ...
            <div className="space-y-3">
              {days.map((day) => {
                const label =
                  ...
                  'Giorno'
                const href = `/home/allenamenti/${encodeURIComponent(planId)}/giorno/${encodeURIComponent(day.id)}`
                return (
                  <Link
                    key={day.id}
                    href={href}
