...
export default function NutrizionistaPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (groupIds.length > 0) {
        ...
        if (!versionsRes.error && versionsRes.data?.length) {
          ...
          pianiInScadenza = allVersions.filter(
            (v) =>
              v.status === PLAN_VERSION_STATUS_ACTIVE && v.end_date && v.end_date <= endDateLimit,
          ).length
        }
      }
      ...
      const versioniMesePrev = allVersions.filter((v) =>
        isInMonth(v.created_at, prevMonthStart, prevMonthEnd),
      ).length
      const progressiSettimanaPrev = progressRows.filter((r) =>
        isInWeek(r.created_at, prevWeekStart, prevWeekEnd),
      ).length
      ...
      const progressi_growth =
        progressiSettimanaPrev > 0
          ? Math.round(
              ((progressiSettimana - progressiSettimanaPrev) / progressiSettimanaPrev) * 1000,
            ) / 10
          : 0
      ...
      progressRows.forEach((r) => {
        if (r.created_at && !lastProgressByAthlete.has(r.athlete_id))
          lastProgressByAthlete.set(r.athlete_id, r.created_at)
      })
      ...
      allVersions.slice(0, 15).forEach((v) => {
        ...
        const name = aid
          ? [profilesMap.get(aid)?.nome, profilesMap.get(aid)?.cognome].filter(Boolean).join(' ') ||
            'Atleta'
          : 'Atleta'
        activityItems.push({
          type: 'version',
            ...
            <>
                ...
                <div className="flex items-center gap-2.5 mt-0.5">
                  <div className="min-w-0">
                    <h2 className="text-base sm:text-lg font-bold text-text-primary leading-tight">
                      I tuoi KPI Nutrizione
                    </h2>
                    <p className="text-text-secondary text-xs sm:text-sm leading-snug">
                      Metriche chiave per i tuoi atleti collegati.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-lg border border-teal-500/30 bg-background-secondary/80 p-3">
                  ...
                  <p
                    className={`text-[11px] sm:text-xs mt-0.5 tabular-nums ${data.growth.versioni_growth >= 0 ? 'text-teal-400' : 'text-red-400'}`}
                  >
                ...
                <div className="rounded-lg border border-teal-500/30 bg-background-secondary/80 p-3">
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-text-primary tabular-nums mt-0.5">
                    {data.atletiSenzaPiano}
                  </p>
                  {data.atletiSenzaPiano > 0 && (
                    <p className="text-[11px] sm:text-xs mt-0.5 text-amber-400">
                ...
                <div className="rounded-lg border border-teal-500/30 bg-background-secondary/80 p-3">
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-text-primary tabular-nums mt-0.5">
                    {data.pianiInScadenza}
                  </p>
                  {data.pianiInScadenza > 0 && (
                    <p className="text-[11px] sm:text-xs mt-0.5 text-amber-400">
                      Rinnovi da pianificare
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="rounded-lg border border-teal-500/20 bg-background-secondary/50 overflow-hidden">
                  {data.attivitàRecent.length === 0 ? (
                    <p className="px-3 py-4 text-center text-text-secondary text-xs sm:text-sm">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-text-secondary text-[11px] sm:text-xs">
                              {e.type === 'version' &&
                                `Nuova versione${e.versionNumber != null ? ` v${e.versionNumber}` : ''}`}
                              {e.type === 'progress' && 'Caricato peso'}
                              {e.type === 'adjustment' &&
                                (e.reason ? `Adattamento: ${e.reason}` : 'Adattamento')}
                            </span>
                          </div>
                          <span className="text-text-muted text-[11px] sm:text-xs shrink-0">
                            {formatTimeAgo(e.at)}
                          </span>
                        </li>
                      ))}
              ...
              <div className="rounded-lg border border-teal-500/30 bg-background-secondary/80 p-3 flex items-center justify-between gap-2">
                ...
                <div className="text-right shrink-0">
                  <p className="text-base sm:text-lg font-bold text-text-primary tabular-nums">
                    {data.versioniMese} Versioni
                  </p>
                  <p
                    className={`text-[11px] sm:text-xs tabular-nums ${data.growth.versioni_growth >= 0 ? 'text-teal-400' : 'text-red-400'}`}
                  >
                    {formatGrowth(data.growth.versioni_growth)} vs mese scorso
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
                ...
                >
                  ...
                  <div>
                    ...
                    <p
                      className={`text-xs mt-1 tabular-nums ${data.growth.versioni_growth >= 0 ? 'text-teal-400' : 'text-red-400'}`}
                    >
                ...
                >
                  ...
                  <div>
                    <p className="text-2xl font-bold text-teal-400 tabular-nums">
                      {data.progressiSettimana}
                    </p>
                    <p
                      className={`text-xs mt-1 tabular-nums ${data.growth.progressi_growth >= 0 ? 'text-teal-400' : 'text-red-400'}`}
                    >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-text-secondary text-sm">
                        {data.prossimiAppuntamenti[0].type === 'nutrizionista'
                          ? 'Visita'
                          : (data.prossimiAppuntamenti[0].type ?? 'Appuntamento')}{' '}
                        ·{' '}
