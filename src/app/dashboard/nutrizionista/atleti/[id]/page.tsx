...
import { User, ArrowLeft, FileDown, Pencil, FileText, Download, Loader2, MoreVertical, Trash2, ExternalLink } from 'lucide-react'
...
type AdjustmentRow = { id: string; created_at: string | null; adjustment_reason?: string | null; is_automatic?: boolean }
...
export default function NutrizionistaAtletaProfilePage() {
  ...
  const [editingDateCell, setEditingDateCell] = useState<{ versionId: string; field: 'start' | 'end' } | null>(null)
  ...
  const [progressPdfDate, setProgressPdfDate] = useState(() => new Date().toISOString().slice(0, 16))
    ...
    try {
      ...
      if (groupIds.length > 0) {
        const verRes = await nutritionFrom(supabase, NUTRITION_TABLES.planVersions)
          .select('id, plan_id, version_number, status, start_date, end_date, calories_target, protein_target, carb_target, fat_target, pdf_file_path')
          .in('plan_id', groupIds)
      ...
      const dbPaths = new Set((verList as PlanVersionRow[]).map((ver) => ver.pdf_file_path).filter(Boolean) as string[])
      ...
      setVersions(verList)
      const active = verList.find((v) => v.status === PLAN_VERSION_STATUS_ACTIVE || v.status === 'published') ?? null
      ...
      if (athleteUserId) {
        const { data: plData } = await supabase
          .from('progress_logs')
          .select('id, created_at, date, weight_kg, massa_grassa_percentuale, waist_cm, hips_cm, source')
  ...
  const canEditValidity = useCallback((v: PlanVersionRow) => !!v.id && !v.id.startsWith('storage-'), [])
        ...
        if (v.id.startsWith('storage-') && v.storagePath && id && orgId && profileId) {
          ...
          if (existingGroupId) {
            ...
            const { data: newGroup, error: groupErr } = await nutritionFrom(supabase, NUTRITION_TABLES.planGroups)
              .insert({ athlete_id: id, nutrizionista_id: profileId, org_id: orgId, title: (v.fileName ?? 'Piano').replace(/\.pdf$/i, '') || 'Piano' } as Record<string, unknown>)
              .select('id')
              .single()
            if (groupErr || !newGroup) throw new Error(groupErr?.message ?? 'Creazione gruppo fallita')
          ...
          const { error: verErr } = await nutritionFrom(supabase, NUTRITION_TABLES.planVersions)
            .insert({
              plan_id: groupId,
              org_id: orgId,
          ...
          const payload = field === 'start' ? { start_date: value || null } : { end_date: value || null }
          const { error } = await nutritionFrom(supabase, NUTRITION_TABLES.planVersions).update(payload).eq('id', v.id)
          if (error) throw error
        }
  ...
  const handleDeletePlan = useCallback(async () => {
    ...
    try {
      if (v.id.startsWith('storage-') && v.storagePath) {
        ...
        const pathToRemove = v.pdf_file_path && (v.pdf_file_path.startsWith('nutrition-plans/') || v.pdf_file_path.includes('/documents/'))
          ? (v.pdf_file_path.includes('/documents/') ? v.pdf_file_path.split('/documents/')[1] : v.pdf_file_path)
          : null
        const { error } = await nutritionFrom(supabase, NUTRITION_TABLES.planVersions).delete().eq('id', v.id)
        if (error) throw error
  ...
  const handleProgressPdfSave = useCallback(async () => {
    ...
    try {
      const dateVal = progressPdfDate ? new Date(progressPdfDate).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10)
  ...
  }, [athleteUserId, profileId, progressPdfDate, progressPdfWeight, progressPdfBodyFat, progressPdfWaist, progressPdfHip, supabase, loadData])
  ...
  const displayName = [profile?.nome, profile?.cognome].filter(Boolean).join(' ') || `Atleta ${id?.slice(0, 8) ?? ''}`
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          ...
          <div className="rounded-xl border border-border bg-background-secondary/50 p-3 sm:p-4 space-y-4">
            ...
            {anagraficError && (
              <p className="text-sm text-red-400 bg-red-500/10 rounded-lg px-3 py-2">{anagraficError}</p>
            )}

            {anagraficEditMode ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-nome" className="text-text-muted">Nome</Label>
                  <Input
                    id="anagrafic-nome"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-cognome" className="text-text-muted">Cognome</Label>
                  <Input
                ...
                <div className="space-y-1.5 sm:col-span-2 lg:col-span-1">
                  <Label htmlFor="anagrafic-email" className="text-text-muted">Email</Label>
                  <Input
                    id="anagrafic-email"
                    type="email"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-telefono" className="text-text-muted">Telefono</Label>
                  <Input
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-data_nascita" className="text-text-muted">Data di nascita</Label>
                  <Input
                    id="anagrafic-data_nascita"
                    type="date"
                    value={anagraficForm.data_nascita}
                    onChange={(e) => setAnagraficForm((f) => ({ ...f, data_nascita: e.target.value }))}
                    className="bg-background border-border"
                  />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-sesso" className="text-text-muted">Sesso</Label>
                  <Input
                    id="anagrafic-sesso"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-codice_fiscale" className="text-text-muted">Codice fiscale</Label>
                  <Input
                    id="anagrafic-codice_fiscale"
                    value={anagraficForm.codice_fiscale}
                    onChange={(e) => setAnagraficForm((f) => ({ ...f, codice_fiscale: e.target.value.toUpperCase() }))}
                    className="bg-background border-border"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-professione" className="text-text-muted">Professione</Label>
                  <Input
                    id="anagrafic-professione"
                    value={anagraficForm.professione}
                    onChange={(e) => setAnagraficForm((f) => ({ ...f, professione: e.target.value }))}
                    className="bg-background border-border"
                    placeholder="Professione"
                  />
                </div>
                <div className="space-y-1.5 sm:col-span-2 lg:col-span-3">
                  <Label htmlFor="anagrafic-indirizzo" className="text-text-muted">Indirizzo</Label>
                  <Input
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-citta" className="text-text-muted">Città</Label>
                  <Input
                    id="anagrafic-citta"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-cap" className="text-text-muted">CAP</Label>
                  <Input
                    id="anagrafic-cap"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-provincia" className="text-text-muted">Provincia</Label>
                  <Input
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="anagrafic-nazione" className="text-text-muted">Nazione</Label>
                  <Input
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium text-text-primary">{profile?.telefono ?? profile?.phone ?? '—'}</p>
                </div>
                <div>
                ...
                <div className="sm:col-span-2 lg:col-span-3">
                  ...
                  <p className="font-medium text-text-primary">
                    {[profile?.indirizzo ?? profile?.indirizzo_residenza, profile?.citta, profile?.cap, profile?.provincia, profile?.nazione]
                      .filter(Boolean)
                      .join(', ') || '—'}
                  </p>
                </div>
              ...
              <div>
                ...
                <p className="font-medium text-text-primary">
                  {weeklyAnalysisList.length > 0
                    ? new Date((weeklyAnalysisList[0] as { created_at?: string }).created_at ?? '').toLocaleDateString('it-IT')
                    : '—'}
                </p>
              </div>
            </div>
        ...
        <TabsContent value="piani" className="mt-4">
                ...
                {versions.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="p-4 text-center text-text-muted">
                      Nessun piano. Crea un piano da &quot;Nuovo piano&quot; in dashboard o carica un PDF dalla pagina Piani.
                    </td>
                  </tr>
                ) : (
                  versions.map((v) => {
                    const isActive = v.status === PLAN_VERSION_STATUS_ACTIVE || v.status === 'published'
                    const storagePath = v.storagePath ?? (v.pdf_file_path && v.pdf_file_path.startsWith('nutrition-plans/') ? v.pdf_file_path : (v.pdf_file_path && v.pdf_file_path.includes('/documents/') ? v.pdf_file_path.split('/documents/')[1] : null))
                    const fileName = v.fileName ?? (storagePath?.split('/').pop() ?? 'piano.pdf')
                    return (
                      <tr key={v.id} className="border-b border-border/50 hover:bg-background-tertiary/30">
                        <td className="p-3 text-text-muted font-mono text-xs">
                          {v.fileName ?? (v.plan_id ? `${v.plan_id.slice(0, 8)}…` : '—')}
                        </td>
                        <td className="p-3 font-medium text-text-primary">
                          {v.storagePath && v.version_number == null ? 'Documento' : `v${v.version_number ?? '—'}`}
                        </td>
                        <td className="p-3">
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs ${
                              isActive ? 'bg-emerald-500/20 text-emerald-400' : v.status === PLAN_VERSION_STATUS_DRAFT ? 'bg-amber-500/20 text-amber-400' : 'bg-background-elevated text-text-muted'
                            }`}
                          >
                            {isActive ? (v.storagePath ? 'Documento' : 'Attivo') : v.status === PLAN_VERSION_STATUS_DRAFT ? 'Bozza' : v.status}
                          </span>
                        </td>
                        <td className="p-3 text-text-muted">
                          {(canEditValidity(v) || v.id.startsWith('storage-')) && editingDateCell?.versionId === v.id && editingDateCell?.field === 'start' ? (
                            <Input
                              type="date"
                              ...
                              onBlur={(e) => {
                                ...
                                if (val !== (v.start_date ?? '')) void handleInlineDateSave(v, 'start', val)
                                setEditingDateCell(null)
                              }}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                            ...
                            <button
                              type="button"
                              onClick={() => (canEditValidity(v) || v.id.startsWith('storage-')) && setEditingDateCell({ versionId: v.id, field: 'start' })}
                              ...
                              title={canEditValidity(v) || v.id.startsWith('storage-') ? 'Clicca per modificare (scrivi la data o apri il calendario)' : undefined}
                            >
                              {v.start_date ? new Date(v.start_date).toLocaleDateString('it-IT') : '—'}
                            </button>
                          )}
                        </td>
                        <td className="p-3 text-text-muted">
                          {(canEditValidity(v) || v.id.startsWith('storage-')) && editingDateCell?.versionId === v.id && editingDateCell?.field === 'end' ? (
                            <Input
                              type="date"
                              ...
                              onBlur={(e) => {
                                const val = e.target.value
                                if (val !== (v.end_date ?? '')) void handleInlineDateSave(v, 'end', val)
                                setEditingDateCell(null)
                              }}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                            ...
                            <button
                              type="button"
                              onClick={() => (canEditValidity(v) || v.id.startsWith('storage-')) && setEditingDateCell({ versionId: v.id, field: 'end' })}
                              className={`text-left w-full min-w-[100px] py-0.5 rounded hover:bg-background-tertiary ${canEditValidity(v) || v.id.startsWith('storage-') ? 'cursor-pointer' : 'cursor-default'}`}
                              title={canEditValidity(v) || v.id.startsWith('storage-') ? 'Clicca per modificare (scrivi la data o apri il calendario)' : undefined}
                            >
                        ...
                        <td className="p-3 text-text-muted text-xs">
                          ...
                          {(v.protein_target != null || v.carb_target != null || v.fat_target != null) && (
                            <span className="block text-text-tertiary">
                              P{v.protein_target ?? '—'} C{v.carb_target ?? '—'} F{v.fat_target ?? '—'}
                            </span>
                          )}
                        </td>
                        <td className="p-3">
                          ...
                          ) : v.pdf_file_path && (v.pdf_file_path.startsWith('http://') || v.pdf_file_path.startsWith('https://')) ? (
                            <a href={v.pdf_file_path} target="_blank" rel="noopener noreferrer" className="inline-flex text-emerald-400 hover:text-emerald-300">
                              <FileText className="h-4 w-4" />
                            </a>
                          ) : (
                        ...
                        <td className="p-3">
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" aria-label="Azioni piano">
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {!v.id.startsWith('storage-') && (
                                <DropdownMenuItem asChild>
                                  <Link href={`/dashboard/nutrizionista/piani?version=${v.id}`} className="flex items-center">
          ...
          <p className="mt-2 text-text-muted text-xs">
            Modifica date validità con l’icona matita. Apri il PDF o vai alla pagina Piani per altre azioni.
          </p>
        </TabsContent>
        ...
        <TabsContent value="struttura" className="mt-4">
          <div className="rounded-xl border border-border bg-background-secondary/50 px-4 py-6 text-center text-text-secondary text-sm">
            Struttura piano (giorni → pasti → alimenti). Seleziona una versione e visualizza gerarchia. Editor CRUD in sviluppo.
          </div>
        </TabsContent>
        ...
        <TabsContent value="progressi" className="mt-4">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
            ...
            {athleteUserId && (
              <Button size="sm" variant="outline" className="gap-2" onClick={() => { setProgressPdfExtractAttempted(false); setProgressPdfExtracted({}); setProgressPdfOpen(true); }}>
          ...
          <ul className="rounded-xl border border-border divide-y divide-border/50 overflow-hidden">
            {progressList.length === 0 ? (
              <li className="p-4 text-center text-text-muted text-sm">Nessun progresso registrato.</li>
            ) : (
              progressList.map((p) => (
                <li key={p.id} className="flex flex-wrap items-center justify-between gap-3 px-3 sm:px-4 py-3 min-h-[44px] hover:bg-background-tertiary/30 touch-manipulation">
                  <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-sm">
                    ...
                    {p.waist_cm != null && <span className="text-text-secondary">Vita {p.waist_cm} cm</span>}
                    {p.hips_cm != null && <span className="text-text-secondary">Fianchi {p.hips_cm} cm</span>}
                    {p.notes && <span className="text-text-muted">{p.notes}</span>}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {p.source === 'pdf_import' && (
                      <span className="rounded-full bg-teal-500/20 text-teal-400 px-2 py-0.5 text-xs">PDF</span>
                    )}
          ...
          {!athleteUserId && (
            <p className="mt-2 text-amber-500/90 text-xs">Atleta senza account: i progressi non possono essere caricati da qui.</p>
          )}
        </TabsContent>
                  ...
                  <li key={i} className="flex justify-between items-center py-2 border-b border-border/50 last:border-0">
                    <span className="text-text-muted">
                      {(w as { created_at?: string }).created_at
                        ? new Date((w as { created_at: string }).created_at).toLocaleDateString('it-IT')
                        : '—'}
                    </span>
                    <span className="text-text-primary">Dati aggregati (calorie medie, macro vs target)</span>
                  </li>
                ))}
              </ul>
            )}
              ...
              <tbody>
                {adjustmentsList.length === 0 ? (
                  ...
                  adjustmentsList.map((a) => (
                    <tr key={a.id} className="border-b border-border/50 hover:bg-background-tertiary/30">
                      <td className="p-3 text-text-muted">
              ...
              {versions
                ...
                .map((v) => (
                  <li key={v.id} className="flex items-center gap-2 min-h-[44px]">
                    ...
                    <span className="text-text-muted truncate max-w-[200px]">{v.pdf_file_path}</span>
                  </li>
                ))}
      ...
      <Dialog open={!!validityEditRow} onOpenChange={(open) => !open && setValidityEditRow(null)}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <p className="text-sm text-text-muted">Imposta o modifica data inizio e scadenza del piano.</p>
          </DialogHeader>
        ...
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <p className="text-sm text-text-muted">
              Il piano &quot;{deletePlanRow?.fileName ?? (deletePlanRow?.version_number != null ? `v${deletePlanRow.version_number}` : 'documento')}&quot; verrà eliminato definitivamente. Questa azione non si può annullare.
            </p>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeletePlanRow(null)} disabled={deletePlanLoading}>
              Annulla
            </Button>
            <Button
              variant="destructive"
              ...
              className="min-h-[44px]"
            >
              {deletePlanLoading ? <Loader2 className="h-4 w-4 animate-spin mr-1.5" /> : <Trash2 className="h-4 w-4 mr-1.5" />}
      ...
      <Dialog open={progressPdfOpen} onOpenChange={setProgressPdfOpen}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <p className="text-sm text-text-muted">Carica un PDF per estrarre i valori oppure inserisci manualmente.</p>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="rounded-lg border border-border bg-background-tertiary/50 p-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                />
                <Button type="button" variant="outline" size="sm" disabled={!progressPdfFile || progressPdfExtracting} onClick={() => void handleProgressPdfExtract()}>
                  ...
                  {Object.keys(progressPdfExtracted).length > 0 ? (
                    <p className="text-emerald-400/90">
                      Valori estratti: {[
                        ...
                        (progressPdfExtracted.massa_grassa_percentuale ?? progressPdfExtracted.body_fat) && `Massa grassa ${progressPdfExtracted.massa_grassa_percentuale ?? progressPdfExtracted.body_fat}%`,
                        (progressPdfExtracted.vita_cm ?? progressPdfExtracted.waist_cm) && `Vita ${progressPdfExtracted.vita_cm ?? progressPdfExtracted.waist_cm} cm`,
                        (progressPdfExtracted.fianchi_cm ?? progressPdfExtracted.hips_cm) && `Fianchi ${progressPdfExtracted.fianchi_cm ?? progressPdfExtracted.hips_cm} cm`,
                      ].filter(Boolean).join(', ') || '—'}
                    </p>
                  ) : (
                    <p className="text-amber-400/90">Nessun valore riconosciuto nel PDF. Inserisci i dati manualmente nei campi sotto.</p>
                  )}
                </div>
              )}
            </div>
            <div>
              ...
              <Input type="datetime-local" value={progressPdfDate} onChange={(e) => setProgressPdfDate(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label>Peso (kg)</Label>
              <Input type="number" step="0.1" placeholder="es. 72.5" value={progressPdfWeight} onChange={(e) => setProgressPdfWeight(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label>Body fat %</Label>
              <Input type="number" step="0.1" placeholder="es. 18" value={progressPdfBodyFat} onChange={(e) => setProgressPdfBodyFat(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label>Vita (cm)</Label>
              <Input type="number" step="0.1" placeholder="es. 82" value={progressPdfWaist} onChange={(e) => setProgressPdfWaist(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label>Fianchi (cm)</Label>
              <Input type="number" step="0.1" placeholder="es. 98" value={progressPdfHip} onChange={(e) => setProgressPdfHip(e.target.value)} className="mt-1" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setProgressPdfOpen(false)}>Annulla</Button>
            <Button onClick={() => void handleProgressPdfSave()} disabled={progressPdfSaving || !athleteUserId} className="min-h-[44px] bg-teal-600 hover:bg-teal-500">
