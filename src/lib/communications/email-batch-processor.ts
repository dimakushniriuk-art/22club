  ...
  const batchPromises = batch.map(async (recipient) => {
    try {
      ...
      const { data: profile, error: profileError } = await supabase
        .from('profiles')
        .select('email')
        .eq('id', recipient.recipient_profile_id)
        .single()
      ...
      if (emailResult.success) {
        ...
        if (emailResult.error) {
          errors.push(`User ${recipient.recipient_profile_id}: ${emailResult.error}`)
        }
      }
    } catch (err) {
      ...
      failedCount++
      errors.push(`User ${recipient.recipient_profile_id}: ${errorMessage}`)
    }
  })
