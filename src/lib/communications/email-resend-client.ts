...
/** Id del template Resend per reset password (dashboard: password-reset-request) */
export const RESEND_TEMPLATE_PASSWORD_RESET = 'password-reset-request'

/**
 * Invia email tramite Resend usando un template pubblicato.
 * Non usare insieme a html/react nel payload.
 */
export async function sendEmailViaResendTemplate(
  to: string,
  templateId: string,
  ...
  try {
    if (!isResendConfigured()) {
      logger.debug('Email con template simulata (Resend non configurato)', undefined, {
        to,
        templateId,
        subject,
      })
    ...
    logger.info('Invio email tramite Resend (template)', undefined, {
      to,
      templateId,
    ...
    if (error) {
      logger.error('Errore invio email Resend (template)', undefined, {
        to,
        templateId,
        error: error.message || 'Resend API error',
      })
    ...
    logger.info('Email inviata con successo tramite Resend (template)', undefined, {
      to,
      templateId,
      emailId: data?.id,
    })
    ...
    logger.error('Errore durante invio email (template)', error, { to, templateId })
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    }
