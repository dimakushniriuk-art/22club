...
- **Ultimo aggiornamento:** 2026-04-18 (`src/app/dashboard/notifiche/page.tsx`: route dedicata inbox staff allineata a sidebar/header `/dashboard/notifiche`.)
...
| `/dashboard/atleti/[id]/progressi/storico` _(hub panoramica, `AthleteWorkoutsTab` `hubSection=overview`)_ |
...
| `progress/`          | Colonne lettura condivise `progress_photos` (`PROGRESS_PHOTOS_LIST_COLUMNS`)                    |

File spesso usati globalmente: `src/lib/utils.ts`, `src/lib/format.ts`, `src/lib/query-keys.ts`, `src/lib/react-query/post-mutation-cache.ts` (invalidazioni dominio: anagrafica atleta, allenamenti, stats esercizi, clienti, progress analytics, documenti, appuntamenti, pagamenti, progressi; `invalidateProgressPhotosFrequentCache`; `syncAuthContextAfterOwnProfilesRowUpdate` dopo update `profiles` riga utente corrente), `src/lib/audit.ts`, `src/lib/cookie-consent-storage.ts` (`OPEN_COOKIE_PREFERENCES_EVENT`, `requestOpenCookiePreferences`), `src/lib/network-retry.ts` (`withNetworkRetry`, usato in `use-user-settings`), `src/lib/appointment-utils.ts`. **Analytics dashboard:** `src/lib/analytics.ts`, `src/lib/analytics-chart-theme.ts` (colori serie/tooltip griglia grafici statistiche), `src/lib/analytics-export.ts`, `src/lib/analytics-workout-bookings-trend.ts`, `src/lib/trainer-analytics.ts` (KPI trainer, fetch Supabase). **progress_logs (staff vs auth uid):** `src/lib/progress-logs-athlete-scope.ts` (`progressLogsAthleteIdOrFilter`). **`progress_photos` (select liste):** `src/lib/progress/progress-photos-columns.ts`. **UI misurazioni (valori + RangeStatusMeter + lista per data):** `src/components/progressi/misurazioni-values-content.tsx`, `misurazione-valori-by-date-list.tsx`; **regole trend % (ricomposizione):** `src/lib/body-metrics/body-metric-trend-rules.ts`. **Storico workout staff:** `src/hooks/progressi/use-storico-allenamenti-profile.ts`; **UI unificata (KPI + lista + PDF):** `src/components/dashboard/athlete-profile/atleta-storico-allenamenti-panel.tsx` (usata nel tab Allenamenti di `/dashboard/atleti/[id]`). **Storage documenti (preview proxy):** `src/lib/documents.ts`; guida riuso e bucket: `src/lib/DOCUMENTI_STORAGE_PREVIEW.md`. **PDF client:** `src/lib/pdf/` (`buildStandardPdfBlob`, logo, worker `public/pdf.worker.min.mjs`, `public/logo.svg`; anteprima `PdfCanvasPreviewDialog`). **Audit performance Supabase (root repo):** [`PERFORMANCE_SUPABASE_AUDIT.md`](PERFORMANCE_SUPABASE_AUDIT.md) — pattern query/cache, interpretazione `db:analyze-complete`, piano verifica in Dashboard.
...
| Schede / allenamenti      | `dashboard/schede`, `dashboard/allenamenti`, `home/allenamenti/*`, `embed/athlete-allenamenti/*`                                                                                                                                                                                                                                                                                                                      | `src/lib/workouts/`, `src/lib/workout-sets-repair-orphan-log.ts` (serie `workout_log_id` null nella finestra sessione), `src/hooks/use-workouts.ts`, `src/hooks/workout-plans/`, `src/contexts/athlete-allenamenti-preview-context.tsx`, `api/staff/athlete-workout-plans`                                                                                                                                                                                                                                                                                                                                                                                          |
| Marketing                 | `dashboard/marketing/**`, `api/marketing/*`                                                                                                                                                                                                                                                                                                                                                                           | `src/lib/marketing/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Statistiche / analytics   | `dashboard/statistiche`                                                                                                                                                                                                                                                                                                                                                                                               | `src/lib/trainer-analytics.ts`, `src/lib/analytics.ts`, `src/lib/analytics-chart-theme.ts`, `src/lib/analytics-export.ts`, `src/lib/analytics-workout-bookings-trend.ts`, `src/components/dashboard/statistiche-page-content.tsx`, `src/components/dashboard/trainer-statistiche-dashboard.tsx`, `src/components/shared/analytics/trend-chart.tsx`, `src/components/shared/analytics/distribution-chart.tsx`, `src/components/dashboard/export-report-button.tsx`                                                                                                                                                                                                  |
